...
- **Hub polish** — Shop and Gardener models are anchored on spawn; a floating green "GARDENER" sign mirrors the "SHOP" sign.
- **Respawns via House 2 SpawnLocation** — the old runtime `PlayerSpawn` pad (and `GameConfig.SPAWN_POSITION`) was removed; the map now ships an authored `SpawnLocation` named "Spawn Point" inside the House 2 model, and `MapService` points every player's `RespawnLocation` at it, so deaths/resets land exactly where that spawn is placed.
- **Admin panel (testers only)** — `AdminController` (top-left **Admin** button) + `AdminService` let whitelisted testers grant themselves any amount of gold/diamonds (typed or quick-add chips) without grinding. Whitelist is `GameConfig.ADMINS`; the new `AdminGrantCurrency` remote is re-validated server-side every call.
- **HUD/shop fixes** — the backpack pill shows exactly one icon (new artwork when set, else a single emoji, no more doubled image); the shop purchase button renders the coin + price in a centered group with a fixed gap; the shopping-cart emoji was removed from the floating "SHOP" sign (it wasn't loading). New Shop/Gardener/House 2 models are in use.
