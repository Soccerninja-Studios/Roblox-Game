...
4. **Clone this repository**: open VS Code → press `Ctrl+Shift+P` → type `Git: Clone` → press Enter → paste `https://github.com/businessmans1984-group/roblox-game.git` → pick a folder → click **Open** when asked.
...
|----------------|--------|--------------------------------------------------------------------|
| `GrassType`    | String | Which grass type to grow (`Meadow`, `Wild`, `Thicket`, `Mossmire`, `Ironweed`). Default `Wild`. |
...
| Mossmire       | medium    | Sickle+ (or Rusty Shears with Sharp Blades)   | 15                 |
| Thicket      | tough     | Sickle+ / more power                          | 25                 |
...
- The **🛒 Shop** button on the **right side** opens the upgrade menu anytime; you can also just **walk into the market stall** near spawn and it opens automatically (and closes when you leave). A **glowing green circle** on the ground marks the exact area where the shop opens. If your model includes a **ProximityPrompt**, holding the interact key opens the shop too. All upgrades (tools + upgrades) live in that menu.

## The world map (the plains)

The neighborhood has been swallowed by an **endless plains biome**. The map is a **5x5 grid
of 512-stud baseplates** (2,560 studs square) with **your house at the centre**, and five
**concentric rings** of grass radiating outward. `MapService` builds the whole thing
**automatically in code** on server start — you don't place anything in Studio.

| Ring | Zone | Grass key | Grass HP | Radius band | Tile size |
|------|------|-----------|----------|-------------|-----------|
| 1 | Meadow | `Meadow` | 3 | 120–255 | 10 |
| 2 | Wildfields | `Wild` | 6 | 290–480 | 14 |
| 3 | **Mossmire** | `Mossmire` | 12 | 515–725 | 18 |
| 4 | **Thicket** | `Thicket` | 16 | 765–965 | 24 |
| 5 | Ironweed Expanse | `Ironweed` | 40 | 1005–1145 | 30 |

**Zones are gated by capability, not purchase.** Each grass type subtracts flat **armor**
from every swing, so a tool that is too weak simply cannot damage it. There is no gold cost
to unlock a ring — you unlock it by being able to cut it.

The hub sits in a cleared circle at the centre, next to the house: the **Shop** stall on the
right, the **Gardener** deposit stand on the left, and **Sprigman The Questgiver** between
them. Scenery (trees, rocks, bushes) is scattered with a **seeded noise function**, so the
same world generates identically every run.

### Mow to reveal

Scenery models across the map start **buried in the overgrowth**. Mowing uncovers them, so
clearing ground visibly changes the world. This is now a property of cutting grass
anywhere — there are no districts or reveal gates.

### The intro cutscene

With `GameConfig.INTRO_CUTSCENE = true`, new players get a short scripted opening: the kid
is at their desk, their **teddy bear falls out the window**, and they run outside — at which
point the neighborhood is swapped for the plains behind a black fade. The Shop, Gardener,
and Sprigman are held in `ServerStorage` and only revealed after the fade, so nothing pops
in on camera.

> Requires **R6 avatars** to be forced in Game Settings, or the cutscene animations fail
> silently.

### Extending the map by hand

Any part you tag with **`GrassSurface`** (with a `GrassType` attribute, and optionally
`GrassHeight`) grows grass the same way, so you can extend the generated map in Studio
whenever you like. Grass grows from the part's local **+Y top face**. The old demo sandbox
is off (`DEMO_ENABLED = false`).

## Saving & progress (persistence)

Player progress (clippings, tool tier, upgrades, stats) is saved to Roblox **DataStores** and reloaded when a player rejoins. You don't have to do anything in-game — it saves automatically:
...
src/
  server/
    ...
    Services/
      ...
      UpgradeService.luau      # tool tiers + upgrades purchases
